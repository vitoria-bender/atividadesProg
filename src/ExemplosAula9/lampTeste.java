package ExemplosAula9;

public class lampTeste {
    public static void main(String[] args) {

        Lampada lampadaUm = new Lampada();

        lampadaUm.marca = "philips";
        lampadaUm.modelo = "LED";
        lampadaUm.cor = "amarela";
        lampadaUm.ligar();
        lampadaUm.desligar();

        Lampada lampadaDois = new Lampada();

        lampadaDois.marca = "philips";
        lampadaDois.modelo = "LED";
        lampadaDois.cor = "amarela";
        lampadaDois.ligar();
        lampadaDois.desligar();

        Lampada lampadaTres = new Lampada();

        lampadaTres.marca = "philips";
        lampadaTres.modelo = "LED";
        lampadaTres.cor = "amarela";
        lampadaTres.ligar();
        lampadaTres.desligar();

        Lampada lampadaQuatro = new Lampada();

        lampadaQuatro.marca = "philips";
        lampadaQuatro.modelo = "LED";
        lampadaQuatro.cor = "amarela";
        lampadaQuatro.ligar();
        lampadaQuatro.desligar();

        Lampada lampadaCinco = new Lampada();

        lampadaCinco.marca = "philips";
        lampadaCinco.modelo = "LED";
        lampadaCinco.cor = "amarela";
        lampadaCinco.ligar();
        lampadaCinco.desligar();
    }
}
